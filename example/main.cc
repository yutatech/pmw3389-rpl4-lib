...
int main() {
  ...
  sensor.UploadFirmware(firmware_data, sizeof(firmware_data));
  std::cout << "size of firmware: " << sizeof(firmware_data) << " bytes" << std::endl;
  sensor.RunSROMSelfTest();
  ...
  std::cout << "SROM ID: 0x" << std::hex << static_cast<int>(sensor.ReadSromId())
            << std::dec << std::endl;

  uint8_t observation;
  sensor.ReadRegister(pmw3389_rpl4_lib::Register::Observation, &observation);
  std::cout << "Observation Register: 0x" << std::hex << static_cast<int>(observation)
            << std::dec << std::endl;
  ...
  // wait for enter key
  std::cout << "Press Enter to start reading motion data..." << std::endl;
  std::cin.get();
